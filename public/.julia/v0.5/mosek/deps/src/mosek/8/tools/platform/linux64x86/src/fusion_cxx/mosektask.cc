...
namespace mosek
{
                        ...
                        const long long * linf)
  {
    ...
    if (! self->breakflag &&
        self->infuserfunc)
        self->infuserfunc(self->infhandle,caller,dinf,iinf,linf);

    if (self->datacbuserfunc)
        self->datacbuserfunc(caller,dinf,iinf,(int64_t*)linf);
  ...
  namespace LinAlg
  {
    ...
    void gemv
      ( bool     transa, 
        ...
        std::shared_ptr<monty::ndarray<double,1>> y)
    {
      int m = transa ? y->size() : x->size();
      int n = transa ? x->size() : y->size(); 
