...
namespace mosek
{
  ...
  public:
    ...
    typedef std::function<void(void * handle, MSKcallbackcodee,const double*,const int*,const long long*)> infhandler_t;
  private:
    bool         breakflag;
    MSKtask_t    task;
  ...
  public:
    ...
    void setCallbackFunc(datacbhandler_t userfunc)                        { datacbuserfunc = userfunc; }
    ...
    void removeStreamFunc()                     { infuserfunc = nullptr; }
    ...
    int appendcones(MSKconetypee ct, int numcone, int conesize, int firstvar, int d0, int d1);
    ...
    void boundslice(MSKaccmodee acc, int first, int last, const MSKboundkeye * bk, const double * lb, const double * ub);
    void boundlist (MSKaccmodee acc, size_t num, const int * sub,     const MSKboundkeye * bk, const double * lb, const double * ub);
